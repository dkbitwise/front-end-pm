...
class Fep_Menu {
	...
	function menu() {
		...
		foreach ( $this->get_menu() as $menu_array ) {
			...
			$menu .= sprintf( '<a id="%1$s" class="%2$s" href="%3$s">%4$s</a>', $menu_array['id'], fep_sanitize_html_class( $class ), $menu_array['url'] ? esc_url( $menu_array['url'] ) : fep_query_url( $menu_array['action'] ), strip_tags( $menu_array['title'], '<span>' ) );
		}
		echo $menu;
	}
